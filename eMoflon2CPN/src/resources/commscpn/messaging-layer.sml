          ...
          in
            ...
            (
              let 
                ...
                val header = Word8Vector.fromList(Word8.fromInt(255)::[]);

                (* .... get the next 127 bytes of data .... *)
                val payload = Word8Vector.extract(byte_stream,0,SOME(127));

                (* .... create a packet .... *)
                val packet = Word8Vector.concat(header::(payload::[]));
              in
                ...
                segmentAndSend(Word8Vector.extract(byte_stream,127,NONE))
              end
            )
