 ...
 *    - renamed signature from NEW_SOCK_UTIL to COMMS_LAYER
   30/09/2002: accept function changed to allow non-local connections.
   10/06/2003: Added canreceive function to check if date is available on a socket.
 *)
...
structure CommsLayer : COMMS_LAYER =
  struct
    ...
    * 
    * Input: (port:int) The port number to listen to.
    * Output: (INetSock.inet stream_sock_passive) The passive server
    *           socket
    *)

    fun bindINetStrm port = 
      let
	  (* Create a server socket *)
	  val server_socket = INetSock.TCP.socket()
	  
         (* Bind the socket *)
	  val _ = Socket.bind(server_socket, INetSock.any port)
     
	  (* Listen for incoming connections with a queue limit on 5 
           * for incoming connection requests.*)
	  val _ = Socket.listen(server_socket, 5)
	  val _ = INetSock.TCP.setNODELAY(server_socket, true)
      in
        (* Return the opened socket. *)
        server_socket
      end
    ...
    *
    * Input: (string) The host name of the server (currently NOT used). 
    *        (int) The port number to lsiten to for connection requests.
    * Output: (InetSock.inet stream_sock_active) The opened TCP socket.
    *)


    fun accept (host, port) = 
      let
	  (* Create a socketand bind *)
	  val server_socket = bindINetStrm port;
	  (* Local function to accept a connection request. *)
	  val (socket_to_client,addr_of_client) = Socket.accept (server_socket);
						       
	   (* Close the server socket and return the opened TCP socket. *)
          val _ =  Socket.close(server_socket);
      in
	  socket_to_client
      end
    ...
    *)
    ...
    fun send (sock, vec) = 
      let
        ...
        fun sendBytes i = 
          Socket.sendVec (sock, {buf=vec, i=i, sz=NONE})
   ...
   fun canreceive (socket) = 
	let
          (* -- find io descriptor -- *)
	  (* val with_iodesc = (OS.IO.pollToIODesc(Socket.pollDesc socket),
	     socket);*)
          val to_poll = (OS.IO.pollIn o OS.IO.pollOut o Socket.pollDesc) socket
          val info_list = OS.IO.poll ([to_poll],SOME(Time.zeroTime))
	in	
	  OS.IO.isIn (List.hd info_list)
        end;
